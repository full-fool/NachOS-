...
class Semaphore {
  public:
    ...
    bool noWaiting() {return queue->IsEmpty();}
...
class Barrier{
public:
    Barrier(char* debugName, int _threadName);
    ~Barrier();
    char *getName()  { return (name); }
    void Wait();
private:
...
public:
    ...
    void ReleaseLock();
    void AcquireWLock();
    int TryRLock();                 //returns 0 if reader lock is available, returns -1 otherwise
    int TryWLock();                 //returns 0 if writer lock is available, returns -1 otherwise
    //void ReleaseWLock();
