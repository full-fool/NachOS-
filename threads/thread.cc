...
Thread::~Thread()
{
    ...
    //printf("currentThread is %s and this thread is %s\n", currentThread->getName(), this->getName());
...
Thread::Fork(VoidFunctionPtr func, int arg)
{
    ...
    {
        printf("%s\n", "Sorry, too much threads forked!");
        FreeTid(tid);
        return;
    }
    printf("Thread forked with name %s and thread id %d and user id %d and priority level %d\n", getName(), tid, uid, priority);
    //printf("In fork and the tidUse[1] is %d\n", tidUse[1]);
    ...
    //printf("Fork returned with tid %d and tidUse[1] is %d\n", tid, tidUse[1]);
}    
...
Thread::Finish ()
{
    ...
    threads[tid] = NULL;
    //printf("Thread with tid %d finished\n", tid);
...
Thread::Yield ()
{
    ...
    //printf("nextThread is %s and priority is %d, this thread is %s and  priority is %d!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", 
      //  nextThread->getName(), nextThread->getPriority(), this->getName(), this->getPriority());
    //printf("In yield and tidUse[1] is %d\n", tidUse[1]);
    if (nextThread != NULL) {
...
Thread::Suspend()
{
    ...
    {
        userRegisters[i] = machine->registers[i];
    }
    while ((nextThread = scheduler->FindNextToRun()) == NULL)
    interrupt->Idle();  // no one to run, wait for an interrupt
