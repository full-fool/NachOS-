...
main(int argc, char **argv)
{
	...
	argCount = 1;
        if (!strcmp(*argv, "-z"))               // print copyright
            printf (copyright);
#ifdef USER_PROGRAM
          memoryBitmap = new BitMap(32);
        if (!strcmp(*argv, "-x")) {        	// run a user program
	    ASSERT(argc > 1);
            StartProcess(*(argv + 1));
            //printf("start process ends here !!!!!!!!!!!!!!!!!!!!\n");
            argCount = 2;
      ...
      printf("interrupt halt at end of main\n");
    ...
    //printf("Before currentThread finished!\n");
    currentThread->Finish();	// NOTE: if the procedure "main" 
				...
				// it from returning.
    //printf("After currentThread finished!\n");
